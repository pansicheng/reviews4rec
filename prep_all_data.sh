...
python -u preprocess_random_split.py $dataset_name $data_file_path $k_core $perc_reviews $data_store_path
...
python -u make_negative_sets.py $dataset_name $k_core $perc_reviews $data_store_path

# Calling below script will:
#	- Create data for HFT format
echo -e "\e[31m\n\nMaking data for HFT format..\n\n\e[0m"
python -u make_data_for_hft.py $dataset_name $k_core $perc_reviews $data_store_path

# Calling below script will:
#	- Create data for MPCN format
echo -e "\e[31m\n\nMaking data for MPCN format..\n\n\e[0m"
python -u make_data_for_mpcn.py $dataset_name $k_core $perc_reviews $data_store_path
python -u make_neg_data_for_mpcn.py $dataset_name $k_core $perc_reviews $data_store_path
...
python -u make_quick_data.py $dataset_name $k_core $perc_reviews "deepconn" $quick_data_deepconn_path $data_store_path

# Calling below script will:
#	- Create .hdf5 files that create padded reviews for train/test/val sets ONLY for NARRE
#	- These .hdf5 files will result in faster training/evaluation
echo -e "\e[31m\n\nMaking quick data for NARRE..\n\n\e[0m"
python -u make_quick_data.py $dataset_name $k_core $perc_reviews "NARRE" $quick_data_narre_path $data_store_path
